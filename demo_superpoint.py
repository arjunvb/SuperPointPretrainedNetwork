...
class VideoStreamer(object):
    ...
    def __init__(self, basedir, camid, height, width, skip, track):
        self.cap = []
        self.camera = False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--H", type=int, default=120, help="Input image height (default: 120)."
    )
    parser.add_argument(
        "--W", type=int, default=160, help="Input image width (default:160)."
    )
    parser.add_argument(
        "--display_scale",
        type=int,
    ...
    parser.add_argument(
        "--waitkey", type=int, default=1, help="OpenCV waitkey time in ms (default: 1)."
    )
    parser.add_argument(
        "--cuda",
        action="store_true",
    ...
    pkl_path = "/data/speed-dnn//data/track_99.pkl"
    with open(pkl_path, "rb") as f:
        track = pickle.load(f)
    vs = VideoStreamer(opt.input, opt.camid, opt.H, opt.W, opt.skip, track)
    ...
    while True:
        ...
        if opt.show_extra:
            ...
            out = cv2.resize(
                out, (2 * opt.display_scale * opt.W, opt.display_scale * opt.H)
            )
        else:
            out = cv2.resize(
    ...
    imageio.mimsave(f"/data2/speed-static/test_superpoint_track_99.gif", outs, fps=5)
